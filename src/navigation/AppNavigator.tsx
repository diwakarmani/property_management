...
export type RoleKey = 'admin' | 'groupAdmin' | 'realtor' | 'seller' | 'buyer';
...
export const pickRoleKey = (roles: string[] | undefined | null): RoleKey => {
  ...
  if (r.includes('REALTOR_GROUP_ADMIN')) return 'groupAdmin';
...
const MainAppScreen = () => {
  ...
  switch (pickRoleKey(user?.roles)) {
    ...
    case 'groupAdmin':
      return (
        <ProtectedRoute requiredRoles={['REALTOR_GROUP_ADMIN']}>
          <GroupAdminNavigator />
        </ProtectedRoute>
      );
    case 'realtor':
    ...
    <NavigationContainer linking={linking}>
      <Stack.Navigator screenOptions={{ headerShown: false }}>
        {!bootstrapped ? (
          ...
          // First-run gate: location must be picked before the app opens.
